...
{
	var $session_namespace = 'N;.pJ8"w';// Just to be sure not to confuse with any other extension
	...
	function onAfterRoute() {
		$options_to_exclude = array('com_dump','com_jcomments');
		if (in_array(JFactory::getApplication()->input->get('option'),$options_to_exclude)  ) { return true; }
		if (JFactory::getApplication()->input->get('option') == 'com_aicontactsafe' && in_array(JFactory::getApplication()->input->get('task'),array('captcha','message'))  ) { return true; }
		...
		// Перевіряємо, чи відбувся перехід з не com_users на com_users. Якщо відбувся, то попередній лінк фіксуємо як returnLink.
		// А для цього коже раз ще зберігаємо поточний компонент у сесію, щоби наступного разу порівняти його із вже новим поточним.
		...
		$session->set('previous_component', $jinput->get('option'), $this->session_namespace);
		//if ($jinput->get('option') == $this->component && $prev_component == $this->component ) { return true; }

		$prev_link = $session->get('previous_link', '', $this->session_namespace);


		$url = $jinput->get('return',false);
		 if ($jinput->get('option') == $this->component) {
			$url = $prev_link;
		}
		else if ($url) {
		...
		if ($jinput->get('option') == $this->component && $prev_component != $this->component ) {
	...
	function onUserAfterSave($data, $isNew, $result, $error)	{
		...
		$user->setParam('returnLink', $redirect_url);
		$user->save();
	 ...
	 * @return	bool or void
	 */
	function activateAndLoginUser () {
		if (!$this->params->get('autologinonactivate', 1)) {return;}
		$app	= JFactory::getApplication();
		$jinput = $app->input;
		...
		// If the user is logged in, return them back to the redirect URL.
		//if ($user->get('id')) {
...
//~ dump ($getaffectedrows,'$res');
		...
		// If activation is enabled and we are here if we could
		// activate the account, then perform the login
		// after login restore the password and redirect
	...
	public function onUserLogin($userarr, $options)	{
		...
		if (!empty ($redirect_url)) {
			...
			$app->setUserState('users.login.form.return', $redirect_url);// Doesn't work
