...
class PlgSystemRegistergoback extends \JPlugin
{
	...
	/**
	 * Update some class properties based on plugin settings.
	 *
	 * E.g. if we use RSForm fo user registration, we add corresponding propertiers here.
	 *
	...
	public function __construct(& $subject, $config)
	{
		...
		if ($this->params->get('RSFormIsUsedForUserRegistration', 1))
		{
			$formId = $this->params->get('RSFormId', 0);

			if ($formId > 0)
			{
				$this->treatAsUserComponents['com_rsform'] = ['formId' => $formId];
				$this->excludeOptions['com_rsform'] = ['formId' => $formId];
			}
			else
			{
				$this->treatAsUserComponents['com_rsform'] = [];
				$this->excludeOptions['com_rsform'] = [];
			}
		}
	}
	...
	/*
	Is used for debug. Don't delete the code, just comments.
	*/
	/**
	public function onAfterRender()
	{
		...
		$session = \JFactory::getSession();
		$lastLink = $session->get('lastLink', false, $this->sessionNamespace);
		$lastLink = \Joomla\CMS\Router\Route::_($lastLink, false);
		dump('onAfterRender');
		dump($lastLink, '$lastLink');
	}
	*/
}
