...
trait Helper
{
	 ...
	 * @return boolean
	 */
	public function shouldProcessRequest()
	{
		...
		if (!array_key_exists($option, $this->excludeOptions))
		{
			return true;
		}

		if (empty($this->excludeOptions[$option]))
		{
			return false;
		}

		if (in_array($task, $this->excludeOptions[$option]))
		{
			return false;
		}
	...
	public function isAuthRelatedPage()
	{
		...
		if (in_array($option, $this->treatAsUserComponents))
		{
			return true;
		}
	...
	public function activateAndLoginUser()
	{
		...
		*/
		...
		if ($uParams->get('useractivation') == 1)
		{
			...
			$redirectUrl = \Joomla\CMS\Router\Route::_($redirectUrl);
